 ...
 */
export const appContext = (req, res, next) => {
  try {
    ...
    pool.query(sql, [appSlug], (err, results) => {
      if (err) {
        console.error("appContext DB error:", err);
        return res.status(500).json({ message: "Failed to load app context" });
      }
